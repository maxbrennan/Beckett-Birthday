...
fakeFlashNextPhaseSuite =
    describe "FakeFlashNextPhase"
        ...
        , test "FfCounterOut exits back to the IQ begin screen with the doubled count, once the skip offer has already been made" <|
            \_ ->
                let
                    state =
                    ...
                    ( result, _ ) =
                        update FakeFlashNextPhase { baseModel | screen = FakeFlashCaughtScreen state, iqOfferMade = True }
                in
                result.screen |> Expect.equal (IQTestScreen { questionIdx = 3, totalDings = 20 })
        , test "FfCounterOut shows the one-time skip offer instead, when it hasn't been made yet" <|
            \_ ->
                let
                    state =
                        { questionIdx = 3, originalTotal = 10, displayNumerator = 0, displayDenominator = 20, phase = FfCounterOut }
