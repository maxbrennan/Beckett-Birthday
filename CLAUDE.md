...
4. After the PR is created, remove the worktree with `git worktree remove .claude/worktrees/<branch-name>` (call `ExitWorktree` first if the session is still inside it). This frees the branch — without deleting it — so the user can check it out locally.
