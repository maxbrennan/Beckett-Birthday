...
var $author$project$Main$pKeyDecoder = A2(
	...
	function (_v0) {
		...
		var repeat = _v0.b;
		var tagName = _v0.c;
		return (((key === 'p') || (key === 'P')) && ((!repeat) && ((tagName !== 'INPUT') && (tagName !== 'TEXTAREA')))) ? $elm$json$Json$Decode$succeed($author$project$Main$ToggleIgnoreDisconnect) : $elm$json$Json$Decode$fail('not p');
	},
	A4(
		$elm$json$Json$Decode$map3,
		F3(
			function (key, repeat, tagName) {
				return _Utils_Tuple3(key, repeat, tagName);
			}),
					...
					function (duration) {
						return A2(
							...
							function (currentTime) {
								return $elm$json$Json$Decode$succeed(
									{currentTime: currentTime, duration: duration, id: id});
							},
...
var $author$project$Main$IQTestCountdownScreen = function (a) {
	return {$: 'IQTestCountdownScreen', a: a};
};
...
var $author$project$Main$clearPending = function (model) {
...
var $author$project$Main$dingScheduleGen = A3(
	...
	A2($elm$random$Random$float, $author$project$Main$minDingDelay, $author$project$Main$maxDingDelay),
	A2(
...
var $author$project$Main$iqFail = F2(
...
var $author$project$Main$iqQuestionCount = $author$project$Main$debug ? 10 : 100;
var $author$project$Main$fakeFlashRangeHi = $author$project$Main$debug ? 0.75 : 0.95;
var $author$project$Main$fakeFlashRangeLo = $author$project$Main$debug ? 0.65 : 0.85;
...
var $author$project$Main$iqTestInitGen = function (total) {
	var lo = A2(
		$elm$core$Basics$max,
		0,
		$elm$core$Basics$floor($author$project$Main$fakeFlashRangeLo * total));
	var hi = A2(
		$elm$core$Basics$max,
		lo,
		A2(
			$elm$core$Basics$min,
			total - 1,
			$elm$core$Basics$floor($author$project$Main$fakeFlashRangeHi * total)));
	return A4(
	...
	function (delay, msg, model) {
		return _Utils_update(
			model,
			{
				pending: A2(
					$elm$core$List$cons,
					{fireAt: model.now + delay, msg: msg},
					model.pending)
			});
	});
			...
			case 'Tick':
				...
				if (!model.now) {
					...
					var _v1 = A2(
						...
						function (e) {
							return _Utils_cmp(e.fireAt, t) < 1;
						},
						model.pending);
					var due = _v1.a;
					var stillPending = _v1.b;
				...
				if (connected || model.ignoreDisconnect) {
					...
					if (_v4.$ === 'Just') {
						...
						var restoredScreen = function () {
							var _v9 = saved.screen;
							if (_v9.$ === 'IQTestActiveScreen') {
								var state = _v9.a;
							...
							if (_v7.$ === 'Just') {
								...
								switch (restoredScreen.$) {
									...
									case 'IQTestActiveScreen':
										var state = restoredScreen.a;
										return state.loudPlaying ? $author$project$Main$seekVideo(t) : $elm$core$Platform$Cmd$none;
									default:
										return $elm$core$Platform$Cmd$none;
								}
							} else {
						...
						var rebasedPending = A2(
							$elm$core$List$map,
							function (e) {
								return _Utils_update(
									e,
									{
										fireAt: model.now + A2($elm$core$Basics$max, 500, e.fireAt - saved.savedAt)
									});
							},
							saved.pending);
						...
						$author$project$Main$clearPending(
							_Utils_update(
								model,
								{
									...
									jeopardyId: $elm$core$Maybe$Nothing,
			...
			case 'PlaySong':
				...
				if (_v15.$ === 'BlankScreen') {
					...
					if (_Utils_eq(blankIdx, idx)) {
						var _v16 = $author$project$Main$getQuestion(idx);
						if (_v16.$ === 'Just') {
							var q = _v16.a;
							return $author$project$Main$isVideo(q.song) ? _Utils_Tuple2(
								_Utils_update(
									model,
									{
										screen: A2($author$project$Main$VideoScreen, idx, q.song)
									}),
			...
			case 'TrackEnded':
				...
				if (name === 'jeopardy-theme.mp3') {
					...
					switch (_v17.$) {
						...
						default:
							return _Utils_Tuple2(
								_Utils_update(
									model,
									{jeopardyId: $elm$core$Maybe$Nothing, jeopardyPlaying: false}),
								$elm$core$Platform$Cmd$none);
					}
				} else {
					var baseModel = _Utils_update(
					...
					switch (_v18.$) {
						case 'BlankScreen':
							var idx = _v18.a;
							var _v19 = $author$project$Main$getQuestion(idx);
							if (_v19.$ === 'Just') {
								var q = _v19.a;
								return _Utils_eq(q.song, name) ? _Utils_Tuple2(
									A3(
										$author$project$Main$schedule,
										1000,
										$author$project$Main$ShowQuestion(idx),
										baseModel),
									$elm$core$Platform$Cmd$none) : _Utils_Tuple2(baseModel, $elm$core$Platform$Cmd$none);
							} else {
							...
							return _Utils_Tuple2(
								A3(
									...
									_Utils_update(
										baseModel,
										{
											screen: $author$project$Main$BlankScreen(idx)
										})),
								$elm$core$Platform$Cmd$none);
						default:
							return _Utils_Tuple2(baseModel, $elm$core$Platform$Cmd$none);
				...
				if (_v23.$ === 'QuestionScreen') {
					...
					if (_v24.$ === 'Just') {
						...
						if (A2(
							...
							q.answers)) {
							var nextIdx = idx + 1;
							var _v25 = $author$project$Main$getQuestion(nextIdx);
							if (_v25.$ === 'Just') {
								return _Utils_Tuple2(
									A3(
										$author$project$Main$schedule,
				...
				if (_v28.$ === 'IQTestScreen') {
					...
					return _Utils_Tuple2(
						A3(
							$author$project$Main$schedule,
							1000,
							$author$project$Main$CountdownTick,
							_Utils_update(
								model,
								{
									screen: $author$project$Main$IQTestCountdownScreen(
										{countdown: iqScreen.totalDings, fakeFlashUsed: iqScreen.fakeFlashUsed, in50PercentPhase: iqScreen.in50PercentPhase, initData: initData, questionIdx: iqScreen.questionIdx, totalDings: iqScreen.totalDings})
								})),
						$elm$core$Platform$Cmd$none);
				} else {
					return _Utils_Tuple2(model, $elm$core$Platform$Cmd$none);
			...
			case 'CountdownTick':
				...
				if (_v29.$ === 'IQTestCountdownScreen') {
					...
					if (state.countdown > 1) {
						...
						var _v30 = state.initData;
						var delay = _v30.delay;
						var nextRandom = _v30.nextRandom;
						var fakeFlashPoint = _v30.fakeFlashPoint;
						return _Utils_Tuple2(
							A3(
								$author$project$Main$schedule,
								delay,
								$author$project$Main$DingOccurred,
								$author$project$Main$clearPending(
									_Utils_update(
										model,
										{
											screen: $author$project$Main$IQTestActiveScreen(
												{dingActive: false, dingCount: 0, fakeFlashActive: false, fakeFlashPoint: fakeFlashPoint, fakeFlashUsed: state.fakeFlashUsed, in50PercentPhase: state.in50PercentPhase, isFlashing: false, loudPlaying: false, nextRandom: nextRandom, questionIdx: state.questionIdx, totalDings: state.totalDings})
										}))),
							$elm$core$Platform$Cmd$none);
					}
				} else {
			...
			case 'DingOccurred':
				...
				if (_v32.$ === 'IQTestActiveScreen') {
					...
					return isFake ? _Utils_Tuple2(
						A3(
							$author$project$Main$schedule,
							$author$project$Main$iqWindowDuration,
							$author$project$Main$FakeFlashWindowExpired,
							A3(
								$author$project$Main$schedule,
								$author$project$Main$iqFlashDuration,
								$author$project$Main$DingFlashEnd,
								_Utils_update(
									model,
									{
										screen: $author$project$Main$IQTestActiveScreen(
						...
						A3(
							$author$project$Main$schedule,
							$author$project$Main$iqWindowDuration,
							$author$project$Main$DingWindowExpired,
							A3(
								$author$project$Main$schedule,
								$author$project$Main$iqFlashDuration,
								$author$project$Main$DingFlashEnd,
					...
					if (state.fakeFlashActive) {
						return (!state.fakeFlashUsed) ? _Utils_Tuple2(
							A3(
								$author$project$Main$schedule,
								1000,
								$author$project$Main$FakeFlashNextPhase,
								$author$project$Main$clearPending(
									_Utils_update(
										model,
										{
							...
							$elm$core$Platform$Cmd$none) : A2($author$project$Main$iqFail, model, state);
					} else {
						if (state.dingActive) {
							...
							var triggerLoud = (!stillPunished) && ((!state.loudPlaying) && (newDingCount === 4));
							...
							if (completed) {
								...
								var newState = _Utils_update(
									state,
									{dingActive: false, dingCount: newDingCount, in50PercentPhase: newIn50Percent, totalDings: newTotalDings});
								var newModel = triggerLoud ? A3(
									$author$project$Main$schedule,
									3000,
									$author$project$Main$StartLoudMusic,
									_Utils_update(
										model,
										{
							...
							return A2($author$project$Main$iqFail, model, state);
						}
					}
				} else {
					return _Utils_Tuple2(model, $elm$core$Platform$Cmd$none);
					...
					switch (_v39.$) {
						...
						case 'FfCounterIn':
							return _Utils_Tuple2(
								A3(
									$author$project$Main$schedule,
									$author$project$Main$counterTickMs,
									$author$project$Main$FakeFlashCounterTick,
									_Utils_update(
										model,
										{
											screen: $author$project$Main$FakeFlashCaughtScreen(
...
var $author$project$Main$view = function (model) {
	...
	switch (_v0.$) {
			...
			return A2(
				...
				_List_fromArray(
					[
						...
						_List_fromArray(
							[
								$elm$html$Html$Attributes$id('playing-video'),
								$elm$html$Html$Attributes$src('assets/' + filename),
								$elm$html$Html$Attributes$autoplay(true),
								A2(
								$elm$html$Html$Events$on,
								'ended',
								$elm$json$Json$Decode$succeed(
									$author$project$Main$TrackEnded(filename))),
								...
								_List_fromArray(
									[
										...
										A2(
											$elm$json$Json$Decode$andThen,
											function (key) {
												return (key === 'Enter') ? $elm$json$Json$Decode$succeed($author$project$Main$AnswerSubmitted) : $elm$json$Json$Decode$fail('not enter');
											},
											A2($elm$json$Json$Decode$field, 'key', $elm$json$Json$Decode$string))),
		...
		case 'IQTestCountdownScreen':
			var state = _v0.a;
			var secondsWord = (state.countdown === 1) ? 'second' : 'seconds';
			return $author$project$Main$screen(
				_List_fromArray(
					[
						A2(
						...
						_List_fromArray(
							[
								$elm$html$Html$text(
								'You may start the IQ test in ' + ($elm$core$String$fromInt(state.countdown) + (' ' + (secondsWord + '.'))))
							]))
					]));
		case 'IQTestActiveScreen':
			var state = _v0.a;
			var counter = $elm$core$String$fromInt(state.dingCount) + (' / ' + $elm$core$String$fromInt(state.totalDings));
			var bg = state.loudPlaying ? '#000000' : '#a8c8e0';
			return A2(
				$elm$html$Html$div,
				_List_Nil,
				_List_fromArray(
					[
						...
						_List_fromArray(
							[
								...
								A2($elm$html$Html$Attributes$style, 'width', '100vw'),
								A2($elm$html$Html$Attributes$style, 'height', '100vh'),
								A2($elm$html$Html$Attributes$style, 'background-color', '#00cc44'),
								A2($elm$html$Html$Attributes$style, 'z-index', '9999'),
								A2($elm$html$Html$Attributes$style, 'pointer-events', 'none')
							]),
						_List_Nil) : $elm$html$Html$text(''),
						state.loudPlaying ? A2(
						$elm$html$Html$video,
						_List_fromArray(
							[
								$elm$html$Html$Attributes$id('playing-video'),
								$elm$html$Html$Attributes$src('assets/loud.mp4'),
								$elm$html$Html$Attributes$autoplay(true),
						...
						A2($author$project$Main$screenBg, bg, _List_Nil)
					]));
		case 'FakeFlashCaughtScreen':
			var state = _v0.a;
